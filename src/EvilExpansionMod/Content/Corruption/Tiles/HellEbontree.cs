...
public class HellEbonModTree : ModTree {
    ...
    public override Asset<Texture2D> GetTexture() {
        return Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTree_Tiles.Asset;
    }
    ...
    public override Asset<Texture2D> GetBranchTextures() {
        return Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTreeBranches.Asset;
    }

    public override Asset<Texture2D> GetTopTextures() {
        return Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTreeTops.Asset;
    }

    public override int DropWood() {
        return ModContent.ItemType<CorruptAshwoodItem>();
    }
}

public class HellEbontree : ModTile {
    public override string Texture => Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTree_Tiles.KEY;

    public static GrowTreeSettings GrowSettings = new GrowTreeSettings
    {
        GroundTest = TreeGroundTest,
            ...
            if(Main.netMode != NetmodeID.MultiplayerClient) {
                if(genRand.NextBool(2)) {
                    ...
                    if(Main.tile[i, k] != null) {
                        dropItem = ModContent.ItemType<CorruptAshwoodItem>();
    ...
    public Texture2D GetTreeTopTexture(int tileType, int treeTextureStyle, byte tileColor) {
        var texture2D = Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTreeTops.Asset.Value;
        return texture2D;
    }

    public Texture2D GetTreeBranchTexture(int tileType, int treeTextureStyle, byte tileColor) {
        var texture2D = Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTreeBranches.Asset.Value;
        return texture2D;
    }
    ...
    public class TreeTopRenderTargetHolder : ARenderTargetHolder {
        ...
        public override void Prepare() {
            var asset = Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTreeTops.Asset;
            asset.Wait?.Invoke();
            PrepareTextureIfNecessary(asset.Value);
        }

        public override void PrepareShader() {
            PrepareShader(Key.PaintColor, Settings);
        }
    }
    ...
    public class TreeBranchTargetHolder : ARenderTargetHolder {
        ...
        public override void Prepare() {
            var asset = Assets.Images.Corruption.Tiles.CorruptHellTree.CorruptHellTreeBranches.Asset;
            asset.Wait?.Invoke();
            PrepareTextureIfNecessary(asset.Value);
        }
    ...
    public override IEnumerable<Item> GetItemDrops(int i, int j) {
        int dropItem = ModContent.ItemType<CorruptAshwoodItem>();
        int dropItemStack = 1;
        int secondaryItem = ModContent.ItemType<CorruptAshwoodItem>();
    ...
    private static void EmitLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY) {
    ...
    public static bool TreeGroundTest(int tileType) {
        if(tileType < 0) {
            return false;
        }
    ...
    public static bool GetTreeFoliageData(int i, int j, int xoffset, ref int treeFrame, out int floorY, out int topTextureFrameWidth, out int topTextureFrameHeight) {
        int num = i + xoffset;
        ...
        if(frameY >= 198 && frameX >= 22) {
            ...
            switch(frameX) {
                case 22: {
                        ...
                        if(!GetTreeFoliageData(x, y, num5, ref treeFrame, out int floorY3, out int topTextureFrameWidth3, out int topTextureFrameHeight3)) {
                            return;
                        }
                        EmitLeaves(x, y, grassPosX, floorY3);
                ...
                case 44: {
                        int num21 = x;
                        int num2 = 1;
                        if(!GetTreeFoliageData(x, y, num2, ref treeFrame, out int floorY2, out _, out _)) {
                            return;
                        }
                        EmitLeaves(x, y, num21 + num2, floorY2);
                ...
                case 66: {
                        int num17 = x;
                        int num18 = -1;
                        if(!GetTreeFoliageData(x, y, num18, ref treeFrame, out int floorY, out _, out _)) {
                            return;
                        }
                        EmitLeaves(x, y, num17 + num18, floorY);
