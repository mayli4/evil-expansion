...
internal sealed class CorruptMenuTheme : ModMenu {
    private float menuTimer;

    public override string DisplayName => "Underworld Corruption";
    ...
    public override bool PreDrawLogo(SpriteBatch spriteBatch, ref Vector2 logoDrawCenter, ref float logoRotation, ref float logoScale, ref Color drawColor) {
        UnderworldCorruptionBgRenderer.DrawBackground(spriteBatch, opacity: 1f);
        return true;
    }
}

internal static class UnderworldCorruptionBgRenderer {
    ...
    public static void DrawBackground(SpriteBatch spriteBatch, float opacity) {
        ...
        foreach (var layerIndex in renderSequence) {
            DrawCorruptionUnderworldLayer(spriteBatch, screenOffset, pushUp, layerIndex, opacity);
        }
    }

    private static void DrawCorruptionUnderworldLayer(SpriteBatch spriteBatch, Vector2 screenOffset, float pushUp, int textureArrayIndex, float opacity) {
